<div class="row w-100 d-flex d-around">
    @if(Auth::user()->character->classes->name == $location->house_affiliation)
        @if (Auth::user()->character->gender->name == $location->title->name)
                ...
                @if(session()->get('current_check') == "Başarısız" or session()->get('current_check') == "")
                    <div class="col-md-6 pt-10px pb-10px">
                        <p class="text-center alert ct-bg-{{ $location->short_link }}">
                            {{ $location->name }} bina öğrencileri içindir ve şifrelidir. Giriş yapabilmek için şifre
                            giriniz.
                        </p>
            ...
            @elseif($location->permit == "Girilmez")
                <p class="text-center">
                    Bu {{ $location->sub_location_type->name }} girilmez alandır. Sadece Yetkili ve Görevli kişiler
        ...
        @else
            <p class="text-center alert ct-bg-{{ $location->short_link }}">
                {{ $location->name }} sadece {{ $location->title->name }} olan öğrenciler içindir.
            </p>
        @endif
    @else
        <p class="text-center alert ct-bg-{{ $location->short_link }}">
            {{ $location->location_type->name }} sadece {{ $location->house_affiliation }} öğrencilerine özeldir.
        </p>
    @endif
