...
<div class="col-md-12">
    <div class="table-responsive">
        <table class="table table-bordered">
            ...
            @foreach ($inn_markets as $material)
                <tr>
                    ...
                    @foreach($character_materials->where('character_id', Auth::user()->character->id)
                        ->where('material_id', $material->id) as $character_inn_market)
                        @php
                            $material_status = 1;
                            $current_material_id = $character_inn_market->id;
                            $current_material_count = $character_inn_market->count;
                        @endphp
                    @endforeach
                    <td>
